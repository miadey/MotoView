...
- **Zero-trust E2E encryption — the browser decrypts locally with vetKeys.** A MotoView app can hold **only ciphertext** on-chain while the plaintext exists only in the user's browser tab. An opt-in Rust crypto module (`/motoview-crypto.wasm`, `ic-vetkeys`/BLS12-381 + IBE, fetched on demand) does the crypto; the canister auto-exposes `GET /_motoview/vetkd/public-key` + `POST /_motoview/vetkd/derive` (the vetKey bound to the session caller); and two declarative attributes — `data-mv-encrypt` on a form field, `data-mv-decrypt="<ciphertext>"` on an element — make the glue encrypt before submit and decrypt on render. All crypto is in WASM; no application JavaScript. With `EncStore` (ciphertext-only storage) and `Audit` (append-only server-side log) this is the full zero-trust stack. **Verified in a real browser** on a deployed canister (the served HTML carries only the `IbeCiphertext`; the note round-trips to the exact plaintext locally). See [Zero-Trust](zero-trust.md) and [`examples/vault`](https://github.com/miadey/MotoView).

...
`build` is the heart of the toolchain. It parses your templates and `@code { ... }` blocks and emits a Motoko actor (under `src/Generated/`, with the entry actor as `src/main.mo`). It does **not** deploy — use it in CI or when you want to inspect generated Motoko before shipping.
...
6. **Write output** to `src/Generated/` and the `src/main.mo` actor.
...
- New to the framework? Start with [Getting Started](getting-started.md).
